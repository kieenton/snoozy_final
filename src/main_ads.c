...
#define SPI_BUF_MAX 32
static uint8_t spi_tx[SPI_BUF_MAX];
static uint8_t spi_rx[SPI_BUF_MAX];

static const struct device *pmic_dev = DEVICE_DT_GET(DT_NODELABEL(npm1304_ek_pmic));

static struct gpio_callback shphld_cb_data;

static void shphld_event_cb(const struct device *dev,
                            struct gpio_callback *cb,
                            uint32_t events)
{
    if (events & BIT(NPM13XX_EVENT_SHIPHOLD_RELEASE)) {
        printk("SHPHLD released - entering hibernate\n");
        mfd_npm13xx_hibernate(dev, 0);
    }
}
...
{
    struct spi_buf txb = { .buf = (void *)data, .len = len };
    struct spi_buf_set txs = { .buffers = &txb, .count = 1 };
    struct spi_buf rxb = { .buf = spi_rx, .len = len };
...
{
    spi_tx[0] = cmd;
    int ret = ads_spi_write_bytes(spi_tx, 1);
    k_busy_wait(10);
    return ret;
}
...
{
    spi_tx[0] = (uint8_t)(CMD_RREG | reg);
    spi_tx[1] = 0x00;
    spi_tx[2] = 0x00;

    int ret = ads_spi_transceive_bytes(spi_tx, spi_rx, 3);
    if (ret == 0) {
        *value = spi_rx[2];
    }
...
{
    spi_tx[0] = (uint8_t)(CMD_WREG | reg);
    spi_tx[1] = 0x00;
    spi_tx[2] = value;

    return ads_spi_write_bytes(spi_tx, 3);
}
...
static int ads_write_regs(uint8_t start_reg, const uint8_t *values, size_t count)
{
    ...
    spi_tx[0] = (uint8_t)(CMD_WREG | start_reg);
    spi_tx[1] = (uint8_t)(count - 1);

    for (size_t i = 0; i < count; i++) {
        spi_tx[2 + i] = values[i];
    }

    return ads_spi_write_bytes(spi_tx, 2 + count);
}
...
{
    spi_tx[0] = CMD_RDATA;
    memset(spi_tx + 1, 0, ADS_FRAME_BYTES);

    int ret = ads_spi_transceive_bytes(spi_tx, spi_rx, 1 + ADS_FRAME_BYTES);
    if (ret) {
        return ret;
    }

    memcpy(frame, spi_rx + 1, ADS_FRAME_BYTES);
    return 0;
}
...
static void connected(struct bt_conn *conn, uint8_t err)
{
    ...
    printk("Interval: %d units (%d ms)", 
            info.le.interval,
            info.le.interval * 5 / 4);  // convert to ms
    
    exchange_params.func = exchange_func;
    bt_gatt_exchange_mtu(conn, &exchange_params);  // <-- add this
}
...
int main(void)
{
    ...
    gpio_pin_set_dt(&reset_pin, 0);  // bring chip out of power down NEW LINE
    k_sleep(K_MSEC(100));
    printk("OK: PWDN and RESET high\n");
    ...
    /* SPI diagnostic: write a known value, read it back.
     * 0xA5 back → SPI works, check RESET/PWDN state
     * 0x00 or 0xFF → MISO stuck, check trace/CS routing
     * 0x96 (CONFIG1 default) → MOSI broken, MISO works */
    {
        uint8_t v = 0;
        ads_write_reg(REG_CONFIG1, 0xA5);
        k_sleep(K_MSEC(1));
        ads_read_reg(REG_CONFIG1, &v);
        printk("SPI diag: CONFIG1 write=0xA5 read=0x%02X\n", v);
    }
    ...
    gpio_pin_set_dt(&start_pin, 0);  // active low → pin goes HIGH → START asserted
    k_sleep(K_MSEC(4));

    if (device_is_ready(pmic_dev)) {
        gpio_init_callback(&shphld_cb_data, shphld_event_cb,
                           BIT(NPM13XX_EVENT_SHIPHOLD_RELEASE));
        mfd_npm13xx_add_callback(pmic_dev, &shphld_cb_data);
